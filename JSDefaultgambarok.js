...
var showPostDate = true,
    ...
    labelSorter = "JQuery",
    loadingText = "Loading...",
    totalPostLabel = "Total Videos:",
    jumpPageLabel = "Showing Videos",
    commentsLabel = "Komentar",
    rmoreText = "View &#9658;",
    prevText = "Previus Page",
    nextText = "Next Page",
    siteUrl = "http://www.banglatune.co.uk",
    postsperpage = 6,
    numchars = 0,
...
// On 3 March 2012
// Visit: http://www.dte.web.id/2012/03/daftar-isi-blogger-dengan-navigasi.html?showComment=1369317982121#c4428416619672321239
// -----------------------------------------------------------------------------------------
...
function showPagePosts(json) {
   ...
   for (var i = 0; i < postsperpage; i++) {
       ...
       if ("entry" in json.feed) {
           ...
           postsumm = ("content" in entry) ? entry.content.$t.replace(/<br ?\/?>/ig, " ").replace(/<(.*?)>/g, "") : ""; // Get the post content
           ...
           output += '<div class="iteminside"><div class="imagecrop"><a href="' + posturl + '" target="_parent"><span class="play-button3"></span><img src="' + postimg + '" width="175" height="95"/></a></div>';
           output += '<h6><a href="' + posturl + '" target="_parent">' + posttitle + '</a></h6>';
...
function _create_pagination() {
   ...
   output += ((actualpage > 1) ? '<a title="' + prevText + '" class="prevjson" href="javascript:_init_script(' + parseInt(actualpage - 1) + ')">' + prevText + '</a>' : '<span class="prevjson hidden">' + prevText + '</span>') + '<em style="font:inherit;color:inherit;" class="pagernumber">';
   ...
   output += '</em>' + ((actualpage < starter - 1) ? '<a title="' + nextText + '" class="nextjson" href="javascript:_init_script(' + parseInt(actualpage + 1) + ')">' + nextText + '</a>' : '<span class="nextjson hidden">' + nextText + '</span>');
...
function _init_script(n) {
   ...
   var url = (sortByLabel) ? siteUrl + '/feeds/posts/default/-/' + labelSorter + '?start-index=' + parameter : siteUrl + '/feeds/posts/default/?start-index=' + parameter; // Optional: Sort posts by a specific label
...
// Execute the _init_script() function with parameter as `1` immediately
